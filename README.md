...
We use Poetry for managing our backend Python environment. To setup a Poetry environment for this project, `cd` into `Backend/` and run `poetry install`. When in the `Backend/` directory, run Python commands using `poetry run <command>`.
