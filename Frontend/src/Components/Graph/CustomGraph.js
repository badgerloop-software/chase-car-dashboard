...
export default function CustomGraph(props) {
    ...
    for (const key in props.data) {
      if (key !== "timestamps") {
        if (props.datasets.includes(key)) {
          datasets.push(key);
          datasetButtons.push(
            DatasetButton(key, () => removeDataset(key), currentColors[i])
          );
        }
      }
  ...
  const isInvalidTitle =
    title === "" || title === "custom" || title === "Custom";
  ...
  const cancelAddDatasets = () => {
    for (const key in props.data) {
      ...
      if (key !== "timestamps") {
        ...
        if (
          !document.getElementById(key + "Btn")?.hidden &&
          document.getElementById(key + "Btn")?.getAttribute("adddata") ===
            "true"
        ) {
  ...
  const removeDataset = (datasetLabel) => {
    ...
    const newDatasets = currentDatasets.filter((dataset, idx) => {
      return idx !== datasetIndex;
    });
    const newDatasetButtons = currentDatasetButtons.filter((button, idx) => {
      return idx !== datasetIndex;
    });
  ...
  const populateDataButtons = () => {
    ...
    for (const key in props.data) {
      if (key !== "timestamps") {
        dataButtons.push(
          DataButton(key, currentDatasets.includes(key), currentColors[i])
        );
      }
      i++;
    }
  ...
  const generateColors = () => {
    ...
    const minRGB = 100;
    var r, g, b;
    const rgbIncrement = Math.floor(
      (maxRGB - minRGB) /
        Math.floor(Math.cbrt(Object.keys(props.data ?? { "": null }).length))
    );
    let rgbStrings = [];
  ...
  return (
    <>
      ...
      <Modal onClose={onSaveClose} size="md" isOpen={isSaveOpen}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Name Custom Graph</ModalHeader>
          <ModalBody>
